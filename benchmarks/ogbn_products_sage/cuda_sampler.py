...
class CudaNeighborSampler(torch.utils.data.DataLoader):
    ...
    def _sample(self, batch):
        ...
        for size in self.sizes:
            # print('will call self.quiver.sample_sub with size=%d' % (size))
            result, row_idx, col_idx = self.quiver.sample_sub(n_id, size)
            assert (row_idx.max() < n_id.size(0))
            # print('called self.quiver.sample_sub')
            # info(result, 'n_ids')

            row_idx, col_idx = col_idx, row_idx
            edge_index = torch.stack([row_idx, col_idx], dim=0)

            # print('row_idx : %d %d ' % (row_idx.min(), row_idx.max()))
            # print('col_idx : %d %d ' % (col_idx.min(), col_idx.max()))
            # print('old: %d, new: %d' % (n_id.size(0), result.size(0)))
