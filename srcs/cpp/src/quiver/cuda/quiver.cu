...
public:
  ...
  std::tuple<torch::Tensor, torch::Tensor> sample_kernel(
    ...
    {
      TRACE("prepare");
      thrust::copy(vertices.data_ptr<long>(), vertices.data_ptr<long>() + bs,
                   inputs.begin());
    ...
    {
      {
        ...
        thrust::copy(policy, inputs.begin(), inputs.end(), subset.begin());
      ...
      torch::Tensor col_idx = torch::empty(tot, vertices.options());
      {
        ...
        replicate_fill(bs, counts.data(), seq.data(), row_idx.data_ptr<T>());
...
TorchQuiver
new_quiver_from_edge_index_weight(size_t n, const torch::Tensor &edge_index,
                                  const torch::Tensor &edge_id,
